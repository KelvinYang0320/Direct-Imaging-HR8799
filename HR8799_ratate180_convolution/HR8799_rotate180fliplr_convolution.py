...
def rotation(img,search,a, x,y):
    ...
    for i in range(search):
        ...
        for j in range(search):
            ...
            foo_shift_check_rot=np.fliplr(foo_shift_check)
...
for ind, img in enumerate(imgcube):
    x, y=Center_of_Gravity(img)
    m[ind], n[ind], rot_record[ind]=rotation(img, search ,a, x, y)
    ind_0=ind+1
    print(ind_0,"/136")
...
for ind, img in enumerate(imgcube):
    ...
    plt.pause(1)
    """
    p=convolution(foo, Standard, a, x, y)
    print(ind+1,"/136")
